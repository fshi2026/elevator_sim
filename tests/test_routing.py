...
from routing import compute_etas, try_insert_pair, walk_capacity
...
def test_insert_into_later_sweep_no_positional_restriction():
    ...
    UP request at pickup=25 doesn't fit the FIRST UP sweep (25 > 20),
    but does fit the THIRD UP sweep (between 5 and 40). The "no
    positional restriction for later sweeps" property is what makes
    this work — the previous DOWN sweep's exit at 5 doesn't block
    inserting at floor 25 in the next UP sweep."""
    existing = [
        PickupEvent(passenger_id=10, floor=20),
        DropoffEvent(passenger_id=11, floor=5),
        PickupEvent(passenger_id=12, floor=40),
        DropoffEvent(passenger_id=10, floor=50),
    ]
    pu = PickupEvent(passenger_id=1, floor=25)
    do = DropoffEvent(passenger_id=1, floor=30)
    result = try_insert_pair(
        events=existing, elev_floor=10, elev_pc=1, capacity=10,
        pickup=pu, dropoff=do,
    )
    assert result is not None
    floors = [e.floor for e in result]
    pu_idx = floors.index(25)
    do_idx = floors.index(30)
    # Pickup at 25 sits inside the third UP sweep — after DO11@5 (index 1),
    # before PU12@40 (which moves to index 4 after the insertions).
    assert pu_idx == 2
    assert do_idx == 3
    assert floors == [20, 5, 25, 30, 40, 50]
